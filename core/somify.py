...
def get_roimat(roi_dict, seiz_dict, my_vars, norm='zscore'):
    
    roi_mat = np.empty((0, len(my_vars)))
    cat_dict = {}
    
    recs_with_seiz = [('_').join(key.split('_')[:3]) for key in list(seiz_dict.keys())]
    ...
    for roi_id in sorted(roi_dict.keys()):
        ...
        my_id = ('_').join(roi_id.split('_')[:3])  # recording id
    
        # provided my_id is a recording with seizrues, check whether this roi-id is an actual seizure
        # if yes: 1, else 0
        if not recs_with_seiz.count(my_id) == 0:
            seizTimes = np.vstack([seiz_dict[this_id]['startstop'] for this_id in sorted(seiz_dict.keys()) if this_id.count(my_id) == 1]).T
            rstart, rstop = mr['seiz_int']
            checklist = [1 for sstart, sstop in seizTimes.T if (sstop >= rstart) and (rstop >= sstart)]
            if len(checklist) > 0:
                cat_dict[roi_id] = 1
                # catlist.append(1)
            else:
                cat_dict[roi_id] = 0
                # catlist.append(0)
        # if my_id is a recording without a seizures, all its roi_ids should not be seizures
        else:
            cat_dict[roi_id] = 0 
            # catlist.append(0)
    if norm == 'L2': roi_mat = np.apply_along_axis(lambda x: x / np.linalg.norm(x), 1, roi_mat)
    elif norm == 'zscore':roi_mat = (roi_mat - np.mean(roi_mat, axis=0)) / np.std(roi_mat, axis=0)
    # elif norm=='01'
    return roi_mat, cat_dict
...
def plot_hexsom(kshape, weights, roimat, type='classic', **kwargs):
    ...
    from matplotlib.pyplot import setp, get_cmap, Polygon, Circle
...
def find_seizClust(my_vars,weights,checklist=['b_n'],**kwargs):
    
    maxlist = kwargs['maxlist'] if 'maxlist' in kwargs else ['dur','b_n','l_rate','l_len','isi_cv']
    minlist = kwargs['minlist'] if 'minlist' in kwargs else ['isi_mean','isi_med','isi_std','isi_fano','isi_peak']

    ref_list = []
    for param in checklist:
        if param in maxlist:param_ref = np.argmax(weights[my_vars.index(param)])
        elif param in minlist:param_ref = np.argmin(weights[my_vars.index(param)])
        else: raise ValueError('Param %s unknown - please enter maxlist or minlist' %(param))
        ref_list.append(param_ref)
    if np.all(param==ref_list[0] for param in ref_list): return ref_list[0]
    else: raise ValueError('Parameters in your checklist %s create different references %s'\
                           %(str(checklist),str(ref_list)))
...
class som_run(object):
    ...
    def __init__(self,n_run=1):
        ...
        datestr = str(now.tm_year-2000).zfill(2)+str(now.tm_mon).zfill(2)+str(now.tm_mday).zfill(2)
...
def writeExampleIDdict_toTXT(exampleDict,txtpath):
    '''example dict has format:
    {fignum:{idxOnMap:roiid}}'''
    tf = open(txtpath,'w') 
    for fignum in list(exampleDict.keys()):
        tf.write('#####Figure {} #####\n'.format(str(fignum+1)))
        for exampleId in sorted(exampleDict[fignum].keys()):
            tf.write('{} --> {}\n'.format(exampleId,exampleDict[fignum][exampleId]))
        tf.write('\n')
    tf.close() 
    return 0
